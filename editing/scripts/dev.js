...
try {
  ...
  if (!fs.existsSync(path.join(projectDir, "src"))) {
    error(
      "No src/ directory found. Please ensure you are in a Discraft project directory.",
    );
    process.exit(1);
  }
  ...
  mon.on("crash", () => {
    error(
      "Application crashed - waiting for file changes before restarting...",
    );
  });
} catch (err) {
  if (err.code === "ENOENT") {
    error(
      "Could not determine current working directory. Please ensure you are in a valid directory with proper permissions.",
    );
  } else {
