...
const command = {
  ...
  execute: async (interaction) => {
    ...
    try {
      ...
      await interaction.reply(`AI Response: ${aiResponse.substring(0, 1500)}${aiResponse.length > 1500 ? '...' : ''}`);

    } catch (error) {
      console.error('AI Error:', error);
      await interaction.reply('There was an error while trying to get an AI response.');
    }
  },
};
